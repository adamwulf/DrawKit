...
static NSRect	_boundsOfCurveSegment( const NSPoint _startPoint, const NSPoint _endPoint, const NSPoint _cp1, const NSPoint _cp2 )
{
    NSPoint low, high;
    
    low.x = MIN(MIN(_startPoint.x, _cp1.x), MIN(_cp2.x, _endPoint.x));
    low.y = MIN(MIN(_startPoint.y, _cp1.y), MIN(_cp2.y, _endPoint.y));
    high.x = MAX(MAX(_startPoint.x, _cp1.x), MAX(_cp2.x, _endPoint.x));
    high.y = MAX(MAX(_startPoint.y, _cp1.y), MAX(_cp2.y, _endPoint.y));
